...
from sklearn.metrics import roc_auc_score, roc_curve, accuracy_score  # <-- add accuracy_score
...
def predict_id_probs_and_labels(
    ...
    """
    Returns:
      prob: [N,K] Dirichlet mean alpha/sum(alpha)
      pred: [N] argmax(prob)
    """
    model.eval()
    probs = []
...
def main(a: Args):
    ...
    # --- IMPORTANT: keep ID labels for accuracy (NEW) ---
    ...
    # OOD from TEST only (keep your original design)
    ...
    # -------- ID classification accuracy (NEW) --------
    _, y_pred = predict_id_probs_and_labels(model, X_id, bs=a.batch_size)

...
class Args:
    ...
    # V1 修改了 7/1/2 为 6/2/2
    # split protocol (new: 6/2/2, seed=0)
    split_seed: int = 0
    ...
    real_ood_classes: Tuple[int, ...] = (8, 9)

    # eval-time OOD set (old: from TEST only)
    ood_classes: Tuple[int, ...] = (8, 9, 10, 11)


    ood_calib_classes: Tuple[int, ...] = (8, 9)      # 训练时会用到（low-evidence）
    ood_unseen_classes: Tuple[int, ...] = (10, 11)   # 训练完全不用，只在测试用
...
def evaluate(a: Args) -> None:
    ...
    # V1 修改了
        # --- OOD split (TEST subjects only) ---
    mask_calib = np.isin(yte_all, a.ood_calib_classes)
    X_ood_calib = Xte_all[mask_calib]

    mask_unseen = np.isin(yte_all, a.ood_unseen_classes)
    X_ood_unseen = Xte_all[mask_unseen]

    print(f"Train ID samples: {len(Xtr)}")
    print(f"Test  ID samples: {len(X_id)} (test+val={a.test_plus_val_as_id})")
    print(f"Test OOD calib samples (classes={a.ood_calib_classes}): {len(X_ood_calib)}")
    print(f"Test OOD unseen samples (classes={a.ood_unseen_classes}): {len(X_ood_unseen)}")
    ...
    sod = score_batch_classwise(
        model, X_ood_calib, mu, inv_cov,
        tau=a.tau, use_inv=a.use_inv_weights, inv_p=a.inv_p, bs=a.eval_batch_size
    )

    y_true = np.concatenate([np.zeros(len(X_id)), np.ones(len(X_ood_calib))])

    s_id = sid["evidence_ood"]
    s_ood = sod["evidence_ood"]
    print(f"\n--- OOD set: calib_ood (seen during training) ---")
    ...
    # ============================================================
    sod = score_batch_classwise(
        model, X_ood_unseen, mu, inv_cov,
        tau=a.tau, use_inv=a.use_inv_weights, inv_p=a.inv_p, bs=a.eval_batch_size
    )

    y_true = np.concatenate([np.zeros(len(X_id)), np.ones(len(X_ood_unseen))])

    s_id = sid["evidence_ood"]
    s_ood = sod["evidence_ood"]
    print(f"\n--- OOD set: unseen_ood (never used in training) ---")
    print(f"-S mean ID : {s_id.mean():.4f}  [5,25,50,75,95]={np.percentile(s_id,[5,25,50,75,95]).round(4)}")
    print(f"-S mean OOD: {s_ood.mean():.4f}  [5,25,50,75,95]={np.percentile(s_ood,[5,25,50,75,95]).round(4)}")
    ...
    print(f"\n[fusion_geo params] use_inv={a.use_inv_weights} tau={a.tau} inv_p={a.inv_p} shrink={a.shrink}")
    print("Done.")

...
# ============================================================
# Route-1 v4 (margin): 6/2/2 subject split + low pseudo-OOD (<= real calib-OOD count) + BN-freeze after warmup
#
# Changes vs v1:
...
def margin_evidence_separation_loss(S_id: torch.Tensor, S_ood: torch.Tensor, margin: float) -> torch.Tensor:
    """
    Simple margin separation on Dirichlet strength S = sum(alpha).
    Encourages: mean(S_id) >= mean(S_ood) + margin.
    """
    return F.relu(margin + S_ood.mean() - S_id.mean())
...
class Args:
    ...
    # Margin loss: enforce ID evidence > OOD evidence + margin
    margin_m: float = 1.5
...
def main(a: Args):
    ...
    ckpt_name = f"eegnet_enn_route1_v4_margin_seed{a.seed}_subj{a.subject_seed}_trial{a.trial_seed}.pt"
    ...
    for ep in range(1, a.epochs + 1):
        ...
        for xb, yb in tr_loader:
            ...
            if rf > 0.0:
                ...
                S_ood_list = []  # collect alpha.sum(1) for any OOD samples used in this step
                ...
                if m_real > 0 and a.lambda_real_ood > 0:
                    ...
                    S_ood_list.append(alpha_real.sum(dim=1))
                ...
                if m_pseudo > 0 and a.lambda_pseudo_ood > 0:
                    ...
                    S_ood_list.append(alpha_pseudo.sum(dim=1))
                    loss_pseudo = ood_low_evidence_loss(alpha_pseudo, mode=a.ood_loss_mode)
                    loss = loss + rf * a.lambda_pseudo_ood * loss_pseudo

                # Margin separation: keep ID evidence higher than OOD evidence
                if a.lambda_margin > 0 and len(S_ood_list) > 0:
                    S_ood = torch.cat(S_ood_list, dim=0)
                    loss_margin = margin_evidence_separation_loss(S_id, S_ood, margin=a.margin_m)
                    loss = loss + rf * a.lambda_margin * loss_margin
